...
function mostrarJogos() {
    ...
    jogosFiltrados.forEach(jogo => {
        ...
        // VERIFICAÇÃO E CORREÇÃO DO STATUS E HORÁRIO DO JOGO
        if (jogo.status === "LIVE" || jogo.status === "IN_PLAY") {
            status = "🔴 AO VIVO";
        } else if (jogo.status === "TIMED" || jogo.status === "SCHEDULED") {
            // Converte a data e hora UTC para o fuso horário local do seu celular
            const dataJogo = new Date(jogo.utcDate);
            const hora = String(dataJogo.getHours()).padStart(2, '0');
            const minuto = String(dataJogo.getMinutes()).padStart(2, '0');
            status = `📅 Hoje às ${hora}:${minuto}`;
