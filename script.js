...
document.addEventListener("DOMContentLoaded", () => {
    // Checa se o usuário já entrou na aba atual (sessão ativa)
    const jaEntrouNestaSessao = sessionStorage.getItem("gustavoPlacares_entrou_sessao");

    if (jaEntrouNestaSessao === "true") {
        // Se já clicou antes nessa aba, esconde a tela inicial imediatamente no F5
        if (telaEntrada) {
...
// Configura o clique do botão (que agora exibe o texto "VER PLACARES")
if (btnEntrar) {
    btnEntrar.addEventListener("click", () => {
        // Salva temporariamente que o usuário já entrou (dura até fechar a aba)
