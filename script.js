// ==========================================
// CONTROLE DE ENTRADA E ÁUDIO (ENTRE ABAS / ATUALIZAÇÕES)
// ==========================================
...
// 1. Detecta se o usuário acabou de ATUALIZAR (dar F5) a página
const foiAtualizado = performance.navigation.type === performance.navigation.TYPE_RELOAD;

// 2. Pega o histórico se ele já entrou nesta sessão
const jaEntrouNestaSessao = sessionStorage.getItem("gustavoPlacares_entrou_sessao");

// Se ele já entrou E a página foi apenas atualizada, esconde a tela direto
if (jaEntrouNestaSessao === "true" && foiAtualizado) {
    if (telaEntrada) {
        telaEntrada.style.display = "none";
    }
} else {
    // Se ele fechou o site e abriu de novo, limpamos a memória para o botão reaparecer normalmente
    if (!foiAtualizado) {
        sessionStorage.removeItem("gustavoPlacares_entrou_sessao");
    }
}
...
if (btnEntrar) {
    btnEntrar.onclick = function() {
        // Salva na sessão atual que ele clicou
