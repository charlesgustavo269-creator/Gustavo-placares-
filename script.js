...
async function carregarJogos() {
    ...
    try {
        ...
        if (conteudo.matches && conteudo.matches.length > 0) {
            jogos = conteudo.matches;
            mostrarJogos();
        } else {
            container.innerHTML = `<div style="text-align:center; color:#888; margin-top:50px;">Nenhum jogo encontrado para hoje. 🏟️</div>`;
        }
    } catch (e) { 
        console.error(e);
        document.getElementById("status-msg").innerHTML = "Aguardando sinal do estádio... 📡";
    }
}
...
function mostrarJogos() {
    ...
    jogos.forEach(jogo => {
        ...
        const terminou = jogo.status === "FINISHED";

        // Regra: se acabou e passou de 1 hora, não mostra
        if (terminou && (agora - dataAtualizacao) > umaHoraEmMs) return;
