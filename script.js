...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Verifica se já entrou na sessão
    if (sessionStorage.getItem("gustavoPlacares_entrou_sessao") === "true") {
        if (telaEntrada) telaEntrada.style.display = "none";
    }

    // Botão é a prioridade total aqui
    if (btnEntrar) {
        btnEntrar.addEventListener("click", function() {
            ...
            // Tenta tocar o áudio
            if (audioAbertura) {
                audioAbertura.play().catch(e => console.log("Áudio bloqueado pelo navegador"));
            }
    ...
    // Carrega os jogos apenas após configurar o botão
    carregarJogos();
});
...
async function carregarJogos() {
    const url = `https://api.allorigins.win/raw?url=${encodeURIComponent("https://api.football-data.org/v4/matches")}`;
    try {
...
function mostrarJogos() {
    ...
    let html = "";
    jogos.filter(j => filtro === "ALL" || j.status === filtro).forEach(jogo => {
        html += `<div class="card">${jogo.homeTeam.name} vs ${jogo.awayTeam.name}</div>`;
    });
